...
import { ObservedDispute, ObservedDisputeResponse } from "@/types/observer"
import { getObserverDisputes } from "@/services/Api/observer/observer"
...
const ObserveDispute = () => {
    const [statusFilter, setStatusFilter] = useState<string>("all")
    const [currentPage, setCurrentPage] = useState(1)
    const [pageSize, setPageSize] = useState(10)
    const { address } = useAppKitAccount();
    const queryClient = useQueryClient();
    const { data: disputesData, isLoading, error } = useQuery<ObservedDisputeResponse>({
        queryKey: ['userdisputes', address, currentPage, pageSize, statusFilter],
        queryFn: async () => {
            const response = await getObserverDisputes(currentPage, pageSize, statusFilter);
              ...
              dark:border-zinc-800 dark:hover:bg-zinc-800/50"
                    ...
                    <TableBody>
                        {error ? (
                            ...
                            filteredDisputes.map((dispute: ObservedDispute) => (
                                <TableRow
